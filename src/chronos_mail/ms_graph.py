...
def get_access_token(application_id, client_secret, scopes):
    ...
    if refresh_token:
        # Try to aquire new access token using the refresh token
        ...
        if not authorization_code:
            raise ValueError("Authorization code is emtpy.")
    ...
    if "access_token" in token_response:
        ...
        raise Exception(f"Failed to acquire access token: " + str(token_response))
